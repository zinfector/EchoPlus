...
os.makedirs(os.path.join(app_dir, 'features'), exist_ok=True)
os.makedirs(os.path.join(app_dir, 'ui'), exist_ok=True)

store_js = "export const state = { classData: [], currentLayout: 'list', activeGridId: null, activeGridIndex: -1, searchQuery: '', sortMode: 'custom', isSelectionMode: false, selectedClasses: new Set(), qaData: [], qaDataLoaded: false, isFetchingQA: false, searchData: [] };
...
toast_js = get_chunk('function showToast', '// --- Selection Mode Logic ---').replace('function showToast', 'export function showToast') + "
window.showToast = showToast;
"
with open(os.path.join(app_dir, 'ui', 'toast.js'), 'w', encoding='utf-8') as f: f.write(toast_js)

theme1 = get_chunk('let isDarkMode = false;', 'const mediaQuery = window.matchMedia')
theme2 = get_chunk('const mediaQuery = window.matchMedia', '// --- Selection Mode Logic ---')
...
player = get_chunk('function getPlayerHTML', 'function getGridDropdownHTML').replace('function getPlayerHTML', 'export function getPlayerHTML').replace('async function initEchoPlayer', 'export async function initEchoPlayer').replace('hostname', 'window.EchoContext.hostname').replace('showToast', 'window.showToast')
...
cache_raw = get_chunk('const cachedKey', 'setInterval(() => {')
poller_raw = get_chunk('setInterval(() => {', 'loadEnrollments();')
...
main_js = "import { state, activeDownloads, sectionId, hostname, courseName } from './store.js';
...
" + cache_raw.replace('classData', 'window.EchoState.classData') + poller_raw.replace('classData', 'window.EchoState.classData') + bindings_raw
