import { resolveVideoUrl } from '../../lib/video-resolver.js';\n\nexport function getPlayerHTML(cls) {
    ...
    const getCurrentTime = () => {
        ...
        let max = 0;
        // Get max time of currently playing media that hasn't reached its own end
        active.forEach(m => { 
            let d = getMediaDuration(m);
            if (!m.paused && !m.ended && m.currentTime < d - 0.5 && m.currentTime > max) max = m.currentTime; 
        });  
        // Fallback to absolute max time if all are paused or ended
    ...
    const seekAllTo = (time) => {
        getActiveMedia().forEach(m => {
            let d = getMediaDuration(m);
            if (d > 0) {
                // Clamp to this specific feed's true duration to freeze it at the end while others continue
                m.currentTime = Math.min(time, d);
            } else {
                m.currentTime = time;
            }
        });
    ...
    function updatePlayUI() {
        ...
        isPlaying = getActiveMedia().some(m => !m.paused && !m.ended && m.currentTime < getMediaDuration(m) - 0.5);
    ...
    function togglePlay() {
        ...
        const anyPlaying = active.some(m => !m.paused && !m.ended && m.currentTime < getMediaDuration(m) - 0.5);
        if (anyPlaying) {
            ...
            active.forEach(m => {
                ...
                if (m.currentTime < getMediaDuration(m) - 0.5) {
                    m.play().catch(()=>{});
                }
            });
        }
