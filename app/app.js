...
const courseName = urlParams.get('courseName') || 'Echo360 Course';
...
let isFetchingQA = false;

// --- CACHE INITIALIZATION ---
const cachedKey = `echo360_syllabus_${sectionId}`;
const initialCache = localStorage.getItem(cachedKey);
if (initialCache) {
    try {
        const cachedSyllabus = JSON.parse(initialCache);
        classData = cachedSyllabus.data.map((item, index) => {
        ...
        async function executeSearch(query) {
            ...
            try {
                const res = await fetch(`https://echo360.org/section/${sectionId}/search`, {
                    method: 'POST',
                ...
                if (docs && docs.length > 0) {
                    ...
                    Object.keys(grouped).forEach(lessonId => {
                        ...
                        // Find matching class
                        // Note: docs.lesson.id is the long ID. classData[i].id might be the short ID or long ID.
                        // We'll search by matching any part of the ID just in case.
                        const cls = classData.find(c => lessonId.includes(c.id) || c.id.includes(lessonId)) || {
                            id: lessonId,
        ...
        async function loadQA() {
            if (qaDataLoaded || isFetchingQA) return;
            isFetchingQA = true;
            try {
                const res = await fetch(`https://echo360.org/questions/search`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ sectionId: sectionId, isClassroom: false, sortDirection: 'desc', pageNumber: 0, requested: true })
                });
                const data = await res.json();
                if (data.status === 'ok' && data.data) {

                    // Parse top-level questions
                    qaData = data.data.map(q => {
                    ...
                    const replyPromises = qaData.filter(q => q.responsesCount > 0).map(async q => {
                        try {
        ...
        async function interactQA(action, questionId, bodyData = null) {
            ...
            if (isLikeToggle || isBookmarkToggle) {
                let targetQ = qaData.find(q => q.id === questionId);
                if (!targetQ) {
        ...
        function generateQAHTML(targetLessonId) {
            ...
            const lessonQA = qaData.filter(q => q.lessonId && targetLessonId && (q.lessonId.includes(targetLessonId) || targetLessonId.includes(q.lessonId)));
            ...
            const filtered = qaData.filter(q => q.title.toLowerCase().includes(query.toLowerCase()) || q.body.toLowerCase().includes(query.toLowerCase()));
    ...
    if (cls && cls.hasVideo) {
        ...
        resolveVideoUrl(cls.lessonData, hostname, cls.mediaId).then(resolved => {
    ...
    if (pipBtn) {
        pipBtn.addEventListener('click', async (e) => {
            e.stopPropagation();
            try {
                const resolved = await resolveVideoUrl(cls.lessonData, hostname, cls.mediaId);
                if (resolved && resolved.urls) {
...
;
        ...
        function renderClasses() {
            ...
            let filteredData = classData.filter(cls =>
                cls.title.toLowerCase().includes(searchQuery) ||
                cls.date.toLowerCase().includes(searchQuery) ||
                cls.time.toLowerCase().includes(searchQuery)
            );
        ...
        function toggleDropdown(e, clsId, element) {
            ...
            if (e && (e.target.closest('.list-action') || e.target.closest('.grid-action') || isSelectionMode)) {
                // If in selection mode, clicking the row should ideally toggle the checkbox instead of opening the video
                if (isSelectionMode && e.target.closest('.cursor-pointer')) {
            ...
            if (currentLayout === 'list') {
                ...
                const isOpen = globalDropdown.classList.contains('open') && activeGridId === clsId;
                ...
                const isSameRow = (globalDropdown.classList.contains('open') && activeGridId !== null && globalDropdown.previousElementSibling === allCards[insertAfterIndex]);
                ...
                if (isOpen) {
                    ...
                    const isMovingRight = clickedIndex > activeGridIndex;
...
async function loadData() {
    ...
    try {
        ...
        if (cachedData && cachedTime && (Date.now() - parseInt(cachedTime) < 300000)) {
            ...
            syllabus = await client.fetchSyllabus(sectionId);
        ...
        classData = syllabus.data.map((item, index) => {
            const lesson = item.lesson?.lesson;
            const mediaId = extractMediaId(item);
            ...
            if (index === 0) {
                console.log(`[Echo360 Debug] Sample mapped classData item (first row):`, mappedItem);
            }
            return mappedItem;
        });
        console.log("[Echo360 Debug] Mapped classData. Rendering classes...");
...
async function loadEnrollments() {
    ...
    try {
        ...
        if (enrollments && enrollments.data && enrollments.data.length > 0) {
            ...
            const currentCourse = userSections.find(c => c.sectionId === sectionId);
            if (currentCourse) {
                            ...
                            <ul class="space-y-1 w-full flex flex-col">
                                ${term.courses.map(course => `
                                    <li class="w-full flex flex-col">
                                        <a href="${hostname}/section/${course.sectionId}/home" class="course-link block w-full px-3 py-2 rounded hover:bg-white dark:hover:bg-gray-700 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-600 shadow-sm hover:shadow">
...
setInterval(() => {
    chrome.storage.local.get('echo360_queue_state', ({ echo360_queue_state: state }) => {
        ...
        for (const [id, dl] of Object.entries(state.active || {})) {
            ...
            const cls = classData.find(c => c.id === id) || { titleRaw: 'Video', date: '' };
