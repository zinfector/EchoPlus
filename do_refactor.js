...
['app/data', 'app/features', 'app/ui'].forEach(dir => {
    const d = path.join(__dirname, dir);
    if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
});

fs.writeFileSync(path.join(__dirname, 'app', 'store.js'), 
"export const state = {
" +
...
let toast = getBetween('function showToast(message)', '// --- Selection Mode Logic ---');
toast = toast.replace('function showToast', 'export function showToast');
fs.writeFileSync(path.join(__dirname, 'app', 'ui', 'toast.js'), toast + "
window.showToast = showToast;
");

let theme = getBetween('let isDarkMode = false;', 'const mediaQuery = window.matchMedia');
let theme_end = getBetween('const mediaQuery = window.matchMedia', '// --- Selection Mode Logic ---');
fs.writeFileSync(path.join(__dirname, 'app', 'ui', 'theme.js'), "export " + theme + "
...
sel = sel.replace(/isSelectionMode/g, 'window.EchoState.isSelectionMode')
...
fs.writeFileSync(path.join(__dirname, 'app', 'features', 'selection.js'), "import { state } from '../store.js';
...
dl = dl.replace(/function handle/g, 'export function handle')
...
fs.writeFileSync(path.join(__dirname, 'app', 'features', 'downloads.js'), "import { state, activeDownloads } from '../store.js';
...
search = search.replace('function handleSearch', 'export function handleSearch')
               ...
               .replace(/updateSelectionUI/g, 'window.updateSelectionUI');
fs.writeFileSync(path.join(__dirname, 'app', 'features', 'search.js'), "import { state } from '../store.js';
...
qa = qa.replace('async function loadQA', 'export async function loadQA')
...
fs.writeFileSync(path.join(__dirname, 'app', 'features', 'qa.js'), "import { state } from '../store.js';
...
player = player.replace('function getPlayerHTML', 'export function getPlayerHTML')
               .replace('async function initEchoPlayer', 'export async function initEchoPlayer')
               .replace(/hostname/g, 'window.EchoContext.hostname')
               .replace(/showToast/g, 'window.showToast');
fs.writeFileSync(path.join(__dirname, 'app', 'ui', 'player.js'), "import { resolveVideoUrl } from '../../lib/video-resolver.js';
...
renderer = renderer.replace('function getGridDropdownHTML', 'export function getGridDropdownHTML')
                   ...
                   .replace(/initEchoPlayer/g, 'window.initEchoPlayer');
fs.writeFileSync(path.join(__dirname, 'app', 'ui', 'renderer.js'), "import { state } from '../store.js';
...
loader = loader.replace('async function loadData', 'export async function loadData')
               ...
               .replace(/renderClasses/g, 'window.renderClasses')
               .replace(/showToast/g, 'window.showToast');
fs.writeFileSync(path.join(__dirname, 'app', 'data', 'loader.js'), "import { EchoApiClient } from '../../lib/api-client.js';
...
let cache = getBetween('const cachedKey', 'setInterval(() => {');
let poller = getBetween('setInterval(() => {', 'loadEnrollments();');
let bindings = code.substring(code.indexOf("document.addEventListener('DOMContentLoaded'"));

let mainApp = "import { state, activeDownloads, sectionId, hostname, courseName } from './store.js';
" +
...
" +
"const cachedKey = " + cache.replace(/classData/g, 'window.EchoState.classData') +
"setInterval(() => {" + poller.replace(/classData/g, 'window.EchoState.classData') +
bindings;

fs.renameSync(path.join(__dirname, 'app', 'app.js'), path.join(__dirname, 'app', 'app_old.js'));
fs.writeFileSync(path.join(__dirname, 'app', 'app.js'), mainApp);
